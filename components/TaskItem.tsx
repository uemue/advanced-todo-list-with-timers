...
import { PlayIcon, PauseIcon, RefreshIcon, CheckCircleIcon, CircleIcon, Bars3Icon, TrashIcon } from './icons';
  ...
  useEffect(() => {
    ...
    if (task.timerStatus === TimerStatus.RUNNING) {
      logicIntervalId = window.setInterval(() => {
        const now = Date.now();
        const effectiveElapsedTimeMs = task.accumulatedTime + (task.timerStartTime ? now - task.timerStartTime : 0);
